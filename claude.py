...
def anthropic_enviar_prompt_extrair_conteudo_pub(texto, max_retries=3):
    client = Anthropic()
    system_prompt = """
...
def anthropic_enviar_prompt_especificar_pub_outros(texto, max_retries=3):
    client = Anthropic()
    system_prompt = """
Você é um assistente jurídico especializado em análise de publicações processuais. 
Sua expertise está em identificar rapidamente o tipo de ato processual comunicado em publicações judiciais. 
...
def anthropic_enviar_prompt_especificar_conteudo_pub_defesa(texto, max_retries=3):
    client = Anthropic()
    system_prompt = """
...
def anthropic_extrair_data_audiencia(texto, max_retries=3):
    client = Anthropic()
    system_prompt = """
